...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(
			...
			throws BeansException {
		// 	转化 beanName，可以看下里面的具体逻辑：
		// 如果前缀是&(即工厂 Bean)则去除前缀，如果 Bean 有别名则获取别名
		...
		// 如果拿不到就尝试进行创建
			...
			try {
				...
				// 如果有 @DependsOn 标记，则先创建 dependsOn 的 Bean
	 ...
	 */
	// 初始化BeanWrapper，主要用于向其中添加ConversionService和PropertyEditor属性编辑器
	...
	protected Object getObjectForBeanInstance(
		...
		// 如果是工厂类，且 beanName 不以&开头，则调用工厂类的 getObject()，
		// 其他情况则返回原对象
	...
	protected void registerDisposableBeanIfNecessary(String beanName, Object bean, RootBeanDefinition mbd) {
		...
		// 必须是单例 Bean 以及要满足 requiresDestruction 方法才能注册 DisposableBeanAdapter 实例执行后续销毁方法
